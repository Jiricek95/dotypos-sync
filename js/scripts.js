  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status === "success") {
        ...
        openCenteredWindow(
          "https://admin.dotykacka.cz/client/connect?client_id=" +
            key +
            "&client_secret=" +
            secret +
            "&scope=*&redirect_uri=" +
            baseUrl +
            "/wp-admin/admin-ajax.php?action=dotypos_token_save&state=",
          600,
          800
        );
...
function openCenteredWindow(url, width, height) {
  ...
  var left =
    ...
    ((window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) - width) / 2;
  var top =
    (window.screenY || window.screenTop) +
    ((window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) - height) / 2;
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (cloudid && cloudid !== "undefined" && cloudid !== null) {
        jQuery("#dotypos_cloud").html(
          '<h2>Dotypos ID Vzdálené správy</h2><br /><div><span class="cloudid">' +
            cloudid +
            '</span><span class="dashicons dashicons-trash jl-delete-icon" onclick="ds_delete_integration()"></span></div>'
        );
      }
...
jQuery(document).ready(function ($) {
  ...
  $("#from_dotypos").on("click", function (event) {
    ...
    if (typeof sku !== "undefined" && sku !== null && sku.length > 0) {
      ...
      $button.html(
        '<span class="spinner is-active" style="float:none; margin-left: 5px; visibility: visible;"></span> Načítám...'
      );
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (doAction == "variant_stock_from_dotypos") {
            $($variationForm.find('input[name="variable_stock[' + index + ']"]')).text(
              response.data.new_stock.stock_status
            );
            $($variationForm.find('input[name="variable_stock[' + index + ']"]')).attr(
              "value",
              response.data.new_stock.stock_status
            );
          }
